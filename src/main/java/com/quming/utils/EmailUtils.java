...
public class EmailUtils {
     ...
     */
    public static EmailUtils subject(String subject) throws SendMailException {
        EmailUtils ohMyEmail = new EmailUtils();
     ...
     */
    public EmailUtils from(String nickName) throws SendMailException {
     ...
     */
    public EmailUtils from(String nickName, String from) throws SendMailException {
        try {
    ...
    public EmailUtils replyTo(String... replyTo) throws SendMailException {
    ...
    public EmailUtils to(String... to) throws SendMailException {
        try {
    ...
    public EmailUtils cc(String... cc) throws SendMailException {
        try {
    ...
    public EmailUtils bcc(String... bcc) throws SendMailException {
        try {
    ...
    public EmailUtils bcc(String bcc) throws MessagingException {
        return addRecipient(bcc, Message.RecipientType.BCC);
    }

    private EmailUtils addRecipients(String[] recipients, Message.RecipientType type) throws MessagingException {
        String result = Arrays.asList(recipients).toString().replace("(^\\[|\\]$)", "").replace(", ", ",");
        msg.setRecipients(type, InternetAddress.parse(result));
        return this;
    }

    private EmailUtils addRecipient(String recipient, Message.RecipientType type) throws MessagingException {
    ...
    public EmailUtils html(String html) {
        this.html = html;
        return this;
    }

    public EmailUtils attach(File file) throws SendMailException {
        attachments.add(createAttachment(file, null));
        return this;
    }

    public EmailUtils attach(File file, String fileName) throws SendMailException {
        attachments.add(createAttachment(file, fileName));
        return this;
    }

    public EmailUtils attachURL(URL url, String fileName) throws SendMailException {
    ...
    public static void main(String[] args) {
        EmailUtils.config(EmailUtils.SMTP_163(true), "wosongxiaoqi@163.com", "a18792532544");
        try {
            EmailUtils.subject("逗你玩").from("最爱你的老公","wosongxiaoqi@163.com").to("285774705@qq.com").text("哈哈哈哈，逗宝宝玩").send();
//            EmailUtils.subject("处理结果").from("最爱你的老公","wosongxiaoqi@163.com").to("897044323@qq.com").text("哈哈哈哈，逗宝宝玩").attach(new File("C:\\Users\\zlc\\Pictures\\8.jpg"), "测试图片.jpeg").send();
